...
class Settings:
    def __init__(self, window):
        ...
        widthlabel = Label(window, text="Board Width:")
        widthlabel.grid(row=2, column=0, sticky=E, padx=10)
        lengthlabel = Label(window, text="Board Length:")
        lengthlabel.grid(row=3, column=0, sticky=E, padx=10)
        bomblabel = Label(window, text="Bomb Count:")
        bomblabel.grid(row=4, column=0, sticky=E, padx=10)
        
        # entries for width, height, and bomb count
        widthentry = Entry(window, width=6, textvariable=self.widthnum)
        widthentry.grid(row=2, column=1, sticky=W)
        heightentry = Entry(window, width=6, textvariable=self.heightnum)
        heightentry.grid(row=3, column=1, sticky=W)
        bombentry = Entry(window, width=6, textvariable=self.bombnum)
        bombentry.grid(row=4, column=1, sticky=W)
    ...
    def startgame(self):
        ...
        if int(self.widthnum.get()) < 10:
            ...
            guiroot = GUI(guiroot, self.widthnum.get(), self.heightnum.get(), self.bombnum.get())
